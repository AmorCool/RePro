...
static plist_t parse_dict_node(struct bplist_data *bplist, const char** bnode, uint64_t size)
{
    ...
    for (j = 0; j < data->length; j++) {
        ...
        /* process key node */
        plist_t key = parse_bin_node_at_index(bplist, (uint32_t)index1);
        if (!key) {
            plist_free(node);
            return NULL;
        }
        ...
        /* process value node */
        plist_t val = parse_bin_node_at_index(bplist, (uint32_t)index2);
        if (!val) {
            plist_free(key);
            plist_free(node);
            return NULL;
...
static plist_t parse_array_node(struct bplist_data *bplist, const char** bnode, uint64_t size)
{
    ...
    for (j = 0; j < data->length; j++) {
        ...
        /* process value node */
        plist_t val = parse_bin_node_at_index(bplist, (uint32_t)index1);
        if (!val) {
            plist_free(node);
            return NULL;
        }
...
PLIST_API void plist_from_bin(const char *plist_bin, uint32_t length, plist_t * plist)
{
    ...
    *plist = parse_bin_node_at_index(&bplist, (uint32_t)root_object);
...
PLIST_API void plist_to_bin(plist_t plist, char **plist_bin, uint32_t * length)
{
    ...
    *length = (uint32_t)bplist_buff->len;
