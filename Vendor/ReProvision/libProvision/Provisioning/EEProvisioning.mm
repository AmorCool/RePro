...
- (void)_addOrUpdateApplicationID:(NSString *)applicationIdentifier applicationName:(NSString *)applicationName binaryLocation:(NSString *)binaryLocation systemType:(EESystemType)systemType withCompletionHandler:(void (^)(NSError *, NSString *, NSDictionary *))completionHandler {
    [[EEAppleServices sharedInstance] listAllApplicationsForTeamID:[[EEAppleServices sharedInstance] currentTeamID] systemType:systemType withCompletionHandler:^(NSError *error, NSDictionary *plist) {
        ...
        // Set application-identifier. Handle case where identifier may already contain TeamID prefix.
        NSString *currentTeamID = [[EEAppleServices sharedInstance] currentTeamID];
        NSString *teamIdPrefix = [NSString stringWithFormat:@"%@.", currentTeamID];
        NSString *applicationId;
        if ([identifier hasPrefix:teamIdPrefix]) {
            // identifier already contains TeamID prefix (e.g., "TEAMID.com.app"), use directly.
            applicationId = identifier;
        } else {
            // identifier is bare bundle ID (e.g., "com.app"), add TeamID prefix.
        ...
        [[EEAppleServices sharedInstance] listTeamsWithCompletionHandler:^(NSError *error, NSDictionary *dictionary) {
            // On error, default to free user mode and continue (don't deadlock).
            BOOL isFreeUser = YES;
            NSArray *teams = nil;
            
            if (error) {
                NSLog(@"listTeamsWithCompletionHandler error: %@. Defaulting to free user mode.", error.localizedDescription);
                // Continue with free user defaults instead of deadlocking.
            } else {
