...
/**
 Set by the app delegate to route provisioning-profile registration through the
 root daemon over XPC. Required on RootHide (the app is sandboxed and cannot
 write the root-owned system profile store; MCProfileConnection is also
 neutered), and a useful fallback on rootless where the mobile user cannot
 write /var/Managed Preferences/mobile either. The handler blocks until the
 daemon replies and returns whether the profile landed in the system store.
 */
