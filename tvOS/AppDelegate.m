...
- (void)applicationSigningUpdateProgress:(int)percent forBundleIdentifier:(NSString *)bundleIdentifier {
    ...
    switch (percent) {
        case 100:
            [[RPVNotificationManager sharedInstance] sendNotificationWithTitle:RPVLS(@"Success") body:[NSString stringWithFormat:RPVLS(@"Signed '%@'"), bundleIdentifier] isDebugMessage:NO andNotificationID:nil];
            break;
        case 10:
...
- (void)applicationSigningDidEncounterError:(NSError *)error forBundleIdentifier:(NSString *)bundleIdentifier {
    ...
    [[RPVNotificationManager sharedInstance] sendNotificationWithTitle:RPVLS(@"Error") body:[NSString stringWithFormat:RPVLS(@"For '%@':\n%@"), bundleIdentifier, error.localizedDescription] isDebugMessage:NO isUrgentMessage:YES andNotificationID:nil];
    ...
    if (error) {
        switch (error.code) {
            case RPVErrorNoSigningRequired:
                [[RPVNotificationManager sharedInstance] sendNotificationWithTitle:RPVLS(@"Success") body:RPVLS(@"No applications require signing at this time") isDebugMessage:NO isUrgentMessage:YES andNotificationID:nil];
                break;
            default:
                [[RPVNotificationManager sharedInstance] sendNotificationWithTitle:RPVLS(@"Error") body:error.localizedDescription isDebugMessage:NO isUrgentMessage:YES andNotificationID:nil];
                break;
        }
    }
}
...
- (void)daemonDidRequestCredentialsCheck {
    ...
    if (![RPVResources getUsername] || [[RPVResources getUsername] isEqualToString:@""] || ![RPVResources getPassword] || [[RPVResources getPassword] isEqualToString:@""]) {
        ...
        [[RPVNotificationManager sharedInstance] sendNotificationWithTitle:RPVLS(@"Login Required") body:RPVLS(@"Tap to login to ReProvision. This is needed to re-sign applications.") isDebugMessage:NO isUrgentMessage:YES andNotificationID:@"login"];
    } else {
